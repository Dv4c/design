### Практична робота №2

##### Тема: Робота з простими формами та їх властивостями у Figma. 

#### Завдання:
1. За керівництвом користувача Figma опрацювати розділи 3-6.  
2. Використовуючи просту форму (shape tool)  Rectangle,  створити  зображення за прикріпленим зразком.




#### Хід роботи:![]
Створюю зображення логотипу фігми використовуючи просту форму Rectangle. Для цього додаю фігури, змінюю їм колір та закруглення:
(workshop_2/figma.png)
Рисунок 2.1 - Створений логотип фігми та приклад

Висновок: на данному практичному занятті було використано прості форми для створення зображення за зразком.